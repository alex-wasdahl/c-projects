//player indicated by @, cyborgs indic by digit char (channel #)
// each turn player can move or stay put
// player takes action then must transmit on one particular channel
// each cyborg moves: normally in random dir, but if transmitter works (50% of time) cyborgs on channel will move as commanded
...
bool canMove(const Arena& a, int dir, int r, int c);
...
{
    return m_channel; //done
}

bool Cyborg::isDead() const
{
    return m_health <= INITIAL_CYBORG_HEALTH - 3; //done
}
...
string Arena::moveCyborgs(int channel, int dir)
{
    ...
    for (int i = 0; i < m_nCyborgs; i++) {
        ...
        if (m_cyborgs[i]->isDead())
        {
            ...
            m_cyborgs[m_nCyborgs - 1] = nullptr;
            m_nCyborgs--;
        }
    }
...
bool canMove(const Arena& a, int dir, int r, int c) //similar to attemptMove but does not actually change the row/column of player, only checks to see if moving in a certain dir is possible
{
    if (dir == NORTH) {
        if (!((r - 1) >= 1 && (r - 1) <= a.rows() && c >= 1 && c <= a.cols()))
        {
...
int numberOfSurroundingCyborgs(const Arena& a, int r, int c)//counts number of cyborgs at each surrounding (NESW) position and returns the total
{
    return a.numberOfCyborgsAt(r - 1, c) + a.numberOfCyborgsAt(r + 1, c) + a.numberOfCyborgsAt(r, c - 1) + a.numberOfCyborgsAt(r, c + 1);
}
...
bool recommendMove(const Arena& a, int r, int c, int& bestDir)
{
    int surroundingCyborgs[NUMDIRS];
    surroundingCyborgs[NORTH] = numberOfSurroundingCyborgs(a, r - 1, c); // north
    surroundingCyborgs[EAST] = numberOfSurroundingCyborgs(a, r, c + 1); // east
    surroundingCyborgs[SOUTH] = numberOfSurroundingCyborgs(a, r + 1, c); // south
    surroundingCyborgs[WEST] = numberOfSurroundingCyborgs(a, r, c - 1); // west
    
    int surroundingCyborgsStanding = numberOfSurroundingCyborgs(a, r, c); //standing
    int bestMove = NORTH;
    for (int i = 0; i < NUMDIRS; i++)
    {
        if (surroundingCyborgs[i] < surroundingCyborgs[bestMove])
        {
            bestMove = i;
        }
    }
    if (surroundingCyborgsStanding <= surroundingCyborgs[bestMove])
    {
        return false;
    }
    else { bestDir = bestMove; return true; }
